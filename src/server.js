...
app.post('/generate', async (req, reply) => {
  ...
  try {
    ...
    // Serve through our own /view endpoint so the Content-Type is always correct.
    const base = `${req.protocol}://${req.headers.host}`;
    const viewUrl = `${base}/view/${slug}`;
    return { ok: true, url: viewUrl, raw: data.publicUrl, slug };
  } catch (err) {
    app.log.error(err);
...
// Serve a generated page with the correct Content-Type (fixes Supabase serving HTML as text)
app.get('/view/:slug', async (req, reply) => {
  const { slug } = req.params;
  try {
    const { data, error } = await supabase.storage.from(BUCKET).download(`${slug}/index.html`);
    if (error || !data) return reply.code(404).send('Not found');
    const html = Buffer.from(await data.arrayBuffer());
    reply.header('Content-Type', 'text/html; charset=utf-8');
    return reply.send(html);
  } catch (err) {
    return reply.code(500).send('Error loading page');
  }
});
